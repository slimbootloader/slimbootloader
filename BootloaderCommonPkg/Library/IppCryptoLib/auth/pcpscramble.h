...
*/
#if 0
__INLINE void cpScramblePut(Ipp8u* pScrambleEntry, int dFactor,
                      const Ipp8u* pData, int dataSize)
{
   int i;
   for(i=0; i<dataSize; i++)
      pScrambleEntry[i*dFactor] = pData[i];
}

__INLINE void cpScrambleGet(Ipp8u* pData, int dataSize,
                      const Ipp8u* pScrambleEntry, int dFactor)
{
   int i;
   for(i=0; i<dataSize; i++)
      pData[i] = pScrambleEntry[i*dFactor];
}
#endif
