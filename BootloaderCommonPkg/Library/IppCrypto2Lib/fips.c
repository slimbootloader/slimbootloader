...
IppCrypto2LibFipsSelftestRsaVerifyPkcsv15 ()
{
    ...
    if(!ippcp_is_fips_approved_func(RSAVerify_PKCS1v15_rmf)) {
     ...
     // NOTE: RSA FIPS selftests are not supported with GCC/Clang builds. See
     // README.md ("FIPS Selftest GCC Limitation") for details and upstream
     // link.
#if defined(__GNUC__) && !defined(__INTEL_COMPILER) && !defined(__INTEL_LLVM_COMPILER)
    DEBUG ((DEBUG_WARN, "  RSA FIPS selftest skipped: not supported with GCC/Clang (upstream ipp-crypto limitation)\n"));
...
IppCrypto2LibFipsSelftestRsaVerifyPSS ()
{
    ...
    // RSA FIPS selftests are not supported with GCC/Clang builds. See README.md
