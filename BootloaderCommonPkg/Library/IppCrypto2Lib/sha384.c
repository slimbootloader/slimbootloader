...
#ifdef _L9
/*
 * Save/restore _IPP32E_E9 using pragmas so the restored value always matches
 * what upstream ipp-crypto defined, rather than relying on a hard-coded 256
 * that could silently diverge on future ipp-crypto updates.
 */
#pragma push_macro("_IPP32E_E9")
#undef  _IPP32E_E9
/* pcphashsha512px.c is guarded by "#if !(...(_IPP32E >= _IPP32E_E9))".
 * For L9, _IPP32E=512 which equals _IPP32E_E9=256, so the guard excludes it.
 * Set _IPP32E_E9 to any value > _IPP32E (512) to make the guard pass and
 * force inclusion of the pure-C fallback. The exact value does not matter;
 * it is restored to the upstream value by pop_macro immediately after. */
...
/**
  ...
  Important L9 dependency: choosing the RAM AVX2 path assumes the runtime image
  layout guarantees 32-byte alignment for the SHA512_YMM_K table used by
  pcpsha512l9as.nasm (vmovdqa ymm). For Stage2 TE images, this is ensured by
  the Stage1B PrepareStage2 realignment fix. Keep this dispatch policy and that
  alignment workaround in sync.
...
IPPFUN( const IppsHashMethod*, ippsHashMethod_SHA384Sbl, (void) )
{
    ...
    * PcdFlashBaseAddress and PcdFlashSize are declared UINT32 in
    * BootloaderCorePkg.dec; FixedPcdGet32 is correct and no truncation occurs.
    */
