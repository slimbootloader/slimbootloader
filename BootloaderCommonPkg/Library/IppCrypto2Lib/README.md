...
| **E9** | AVX        | AVX        | AVX      | SSE3      |
| **L9** | SHA-NI     | AVX2 (RAM) / pure-C (XIP) | AVX (E9) | AVX2/SSE3 |
...
**Fix:** `fixup_stage2_avx2_alignment()` in `BuildLoader.py` (run post-build for L9
boards) reads the TE header, computes the misalignment `(te_fd_offset - Adjust) mod 32`,
and inserts 1–31 pad bytes at the TE image boundary inside STAGE2.fd. It also:
