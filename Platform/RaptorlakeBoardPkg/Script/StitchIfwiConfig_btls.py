## @ StitchIfwiConfig.py
#  This is an IFWI stitch config script for BTL-S Slim Bootloader
#
...
stitching ingredients listed in step 2 below, please contact your Intel representative.
  ...
  4. Stitch the final image
       EX:
       Assuming stitching workspace is at D:\Stitch and building ifwi for BTL-S platform
       To stitch IFWI with SPI QUAD mode and Boot Guard profile VM:
         StitchIfwi.py -b vm -p btls -w D:\Stitch -s Stitch_Components.zip -c StitchIfwiConfig.py
...
def get_platform_sku():
    platform_sku ={
        'btls'    :   'Intel(R) AlderLake S Chipset - Consumer - SPI - B0',
    }
    return platform_sku
...
def get_xml_change_list (platform, plt_params_list):
    ...
    if 's06' in plt_params_list:
        xml_change_list.append ([
        ('./BuildSettings/HarnessGlobalData/SelectedRvp',                            'ADL-S DDR5 UDIMM (ADP-S + ADL-S) S03 S06 Fab2'),
