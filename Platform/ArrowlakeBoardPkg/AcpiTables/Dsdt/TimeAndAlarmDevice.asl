...
External(WADT_ACPI_DEV_PATH.ACWA)
External(WADT_ACPI_DEV_PATH.DCWA)
External(WADT_ACPI_DEV_PATH.ACET)
External(WADT_ACPI_DEV_PATH.DCET)
...
Device (AWAC) { // Time and Wake Alarm Device
  ...
  Method (_PRW, 0) { Return (GPRW (WADT_WAKE, 4)) }
  Mutex (RTCL, 0)
  Method (_STA) {
    If (LEqual (\STAS,0)) {
      Return (0x0F)
  ...
  Method (_GCP, 0) {
    ...
    Return (0xB7) // AC/DC wake supported. Get/Set real time features implemented. _GWS returns the correct value from S4/S5 wake. Wakefrom S4 supported for AC/DC
  }
  ...
  Method (_GRT, 0 ,Serialized) {      // Get Real Time
    ...
    // Timeout is set 0.1 second
    //
    While (LAnd (And (REGA, 0x80), LLess (Local1, TOUT))) {
        Stall (10)
        Add (Local1, 10, Local1)
    }
  ...
  Method (_GWS, 1, Serialized) { // Get Wake Alarm Status
    ...
    If (LEqual (Arg0, 0)) { // AC timer
      If (LAnd (LEqual (WADT_ACPI_DEV_PATH.ACWA, 0xFFFFFFFF), And (BIT0, WTTR))) { // AC Timer expired.
        Or (BIT0, Local0, Local0)
        Xor (BIT0, WTTR, WTTR) // Clear AC timer tracker
      }
    }
    ElseIf (LAnd (LEqual (WADT_ACPI_DEV_PATH.DCWA, 0xFFFFFFFF),And (BIT1, WTTR))) { // DC Timer expired.
      Or (BIT0, Local0, Local0)
  ...
  Method (_STP, 2) { // Set Expired Timer Wake Policy
    ...
    // Return 0 - Success, 1 - Failure
    If (LEqual (Arg0, 0)) { // AC Timer
      Store (Arg1, WADT_ACPI_DEV_PATH.ACET)
    } Else { // DC Timer
      Store (Arg1, WADT_ACPI_DEV_PATH.DCET)
    }
    Return (0) // Always success
  }
  Method (_STV, 2, Serialized) { // Set Timer Value
    ...
    // Return 0 - Success, 1 - Failure
    If (LEqual (Arg0, 0)) {      // AC Timer
      Store (Arg1, WADT_ACPI_DEV_PATH.ACWA)
      Or (BIT0, WTTR, WTTR)
    } Else {   // DC Timer
      Store (Arg1, WADT_ACPI_DEV_PATH.DCWA)
      Or (BIT1, WTTR, WTTR)
    }
    Return (0) // Success
  }
  Method (_TIP, 1) { // Expired Timer Wake Policy
    ...
    If (LEqual (Arg0, 0)) { // AC Timer Wake Policy
      Return (WADT_ACPI_DEV_PATH.ACET)
    } Else {                // DC Timer Wake Policy
      Return (WADT_ACPI_DEV_PATH.DCET)
    }
  }
  Method (_TIV, 1) { // Timer Values
    ...
    If (LEqual (Arg0, 0)) { // AC Timer
      Return (WADT_ACPI_DEV_PATH.ACWA)
    } Else {                // DC Timer
      Return (WADT_ACPI_DEV_PATH.DCWA)
    }
  }
} // Device (AWAC)
