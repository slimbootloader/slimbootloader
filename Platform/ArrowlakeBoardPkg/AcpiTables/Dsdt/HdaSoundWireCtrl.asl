...
Device (SNDW) {
  ...
  Method (_INI) {
    ...
    SDWU (SNDW_MASTER_COUNT_NVS_PATH)

    // Update Link Enable property
    // SWQx - PCH NVS variables set in accordance with PchPolicy (AudioLinkSndwX)
    QCFG (LNK0, SNWD0_QUIRK_MASK_NVS_PATH, SNWD0_ACS_NVS_PATH)
    QCFG (LNK1, SNWD1_QUIRK_MASK_NVS_PATH, SNWD1_ACS_NVS_PATH)
    QCFG (LNK2, SNWD2_QUIRK_MASK_NVS_PATH, SNWD2_ACS_NVS_PATH)
    QCFG (LNK3, SNWD3_QUIRK_MASK_NVS_PATH, SNWD3_ACS_NVS_PATH)

    // Update XTAL based properties
    // XTAL - PCH NVS variable with XTAL frequency (0 - 24MHz; 1 - 38.4MHz)
    XCFG (LNK0, XTAL_NVS_PATH)
    XCFG (LNK1, XTAL_NVS_PATH)
    XCFG (LNK2, XTAL_NVS_PATH)
    XCFG (LNK3, XTAL_NVS_PATH)

    // Update DOAIS and DODS based proparties
    // PCH NVS variables set in accordance with PchPolicy
    DCFG (LNK0, SNWD0_DAI_NVS_PATH, SNWD0_DOD_NVS_PATH)
    DCFG (LNK1, SNWD1_DAI_NVS_PATH, SNWD0_DOD_NVS_PATH)
    DCFG (LNK2, SNWD2_DAI_NVS_PATH, SNWD0_DOD_NVS_PATH)
    DCFG (LNK3, SNWD3_DAI_NVS_PATH, SNWD0_DOD_NVS_PATH)
  }
