...
External (\_SB.FND2, IntObj) // EnableFan2Device

Device (TFN4)
{
  Name (_HID, "INTC1048")  // Intel DPTF Fan Device
  Name (_UID, "TFN2")
  Name (_STR, Unicode ("Fan Auto 2"))
  ...
  IndexField (INDX, DATA, ByteAcc, Lock, Preserve)
  {
    ...
    R6CR, 8         // FAN_CTL2 SmartGuardian Automatic Mode control register
  }
  ...
  Store( 0x80 , R16R) // enable smart/auto mode Fan1 - Tmpin3(PECI)
  Store( 0x3F,  R13R)  //Enable Both Fan1 tech and Fan2 tach
  Store( 0xCC , R51R) //Select Fan2 reading temperature and select TempIn1

  //Set the Fan Temp limits
  Store(0x10,R68R) // Set Fan Off temp 16C         0x10              (Note: Fan2 being controlled by Tmp1in VCCcore sensor)
  Store(0x22,R69R) // Set Fan2 Start temp 34C       0x22
  Store(0x3D,R6AR) // Set Fan2 Full temp 61C        0x3D
  Store(0x80,R6BR) // Start Fan 2 PWM < 50%         0x80
  Store(0x18,R6CR) // Set no fan 2 smoothing, slope is 128C/61C-34C or ~4.7         (0x18 is slope 3)
  ...
  Method(_FST,0,Serialized,,PkgObj)
  {
    ...
    Store(0x0E, Index(TFST, 2))  // Show the fan speed value in Auto mode
    Return(TFST)
  } //End TFN4 device
}
