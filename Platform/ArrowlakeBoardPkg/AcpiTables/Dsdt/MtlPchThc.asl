...
Scope (\_SB.PC02) {
  ...
  //
  // should be update after Gpio V2 porting to MTL PCH
  Method (TINT, 1, Serialized) {
    Name (TINR, ResourceTemplate () {
      GpioInt (Level, ActiveLow, ExclusiveAndWake, PullUp, 0x0000, "\\_SB.PGPI", 0x00, ResourceConsumer, INTG ) { 0 }
    })
    CreateWordField (TINR, INTG._PIN, INT1)
    Store (\_SB.PGPI.GNUM (Arg0), INT1)
    Return (TINR)
  }
...
#define GPIO_CONTROLLER_PATH            \_SB.PGPI
  Device (THC0) {
