  ...
  )
{
  ...
  Scope(\_SB.PR00)
  {
    ...
    //        BITS 7:5 has the Num of processors in that module
    //        BITS 4   Die ID.
    ...
    Method(GPSD, 2)
    {
      ...
      If (LAnd(And(\_SB.CFGD, PPM_TURBO_BOOST_MAX), And(\_SB.CFGD, PPM_HWP)))
      {
        ...
        If (HTRO) {
          ...
          //
          Store (ShiftRight (AND (Arg1, 0xE0),5), Index (DerefOf (Index (HPSD, 0)), 4))
          Store (ShiftRight (AND (Arg1, 0xE0),5), Index (DerefOf (Index (SPSD, 0)), 4))

          Store (AND (Arg1, 0x1F), Index (DerefOf (Index (HPSD, 0)), 2)) // Domain
          Store (AND (Arg1, 0x1F), Index (DerefOf (Index (SPSD, 0)), 2)) // Domain
        } Else {
