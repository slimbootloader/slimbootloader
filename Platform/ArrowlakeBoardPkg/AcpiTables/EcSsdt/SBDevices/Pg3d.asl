...
Device (ISCT) {
  ...
  Method (_STA) {
    ...
    //
    ADBG (Concatenate ("PGED = ", ToHexString (PGED)))
    ...
    If (LEqual (PGED, 1)) {
      ...
      If (LEqual (\_SB.PC00.LPCB.H_EC.ECRD (RefOf (\_SB.PC00.LPCB.H_EC.RPWR)),0)) {
        ...
        //
        ADBG ("DC mode : ISCT virtual device present.")
        ADBG (Concatenate ("PG3D Pseudo G3 entry = ", ToHexString (\_SB.PC00.LPCB.H_EC.PG3E)))
        ADBG (Concatenate ("PG3D Pseudo G3 exit  = ", ToHexString (\_SB.PC00.LPCB.H_EC.PG3X)))
        Return (0x0F)
      } Else {
        ADBG ("AC mode : ISCT virtual device hiding.")
        Return (0x00)
      }
    } else {
      ADBG ("ISCT virtual device doesn't exist")
      Return (0x00)
      }
    }
  //
