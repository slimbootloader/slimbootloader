...
{
    return ((MmioRead8 (PCH_PWRM_BASE_ADDRESS + R_PMC_PWRM_GEN_PMCON_B) & B_PMC_PWRM_GEN_PMCON_B_RPS) == 0);
}
  ...
  )
{
  ...
  if (PmcIsRtcBatteryGood () == FALSE) {
    ...
    //
    MmioWrite8 (PCH_PWRM_BASE_ADDRESS + R_PMC_PWRM_GEN_PMCON_B, MmioRead8 (PCH_PWRM_BASE_ADDRESS + R_PMC_PWRM_GEN_PMCON_B) & ~(B_PMC_PWRM_GEN_PMCON_B_RPS) );
  }
