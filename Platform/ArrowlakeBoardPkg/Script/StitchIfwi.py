...
from   security_stitch_help_arl  import *
...
def gen_xml_file(stitch_dir, stitch_cfg_file, btg_profile, plt_params_list, platform, tpm):
    ...
    add_multi_instance_elements(tree, xml_change_list)

    for each in xml_change_list:
        for xml_path, value in each:
            print ("xml_path = ", xml_path, ", value = ", value, sep="")
...
def add_multi_instance_elements(root, xml_change_list):
    # Dynamically add new XML element(s) based on the following XML value (valid for 2 or more):
    # - ./FlashLayout/SubPartitions/EsePackage/DmuSubPartition/num_of_instance_DMUBinary_path
    #   (Adding ./FlashLayout/SubPartitions/EsePackage/DmuSubPartition/DMUBinary<number> XML tag)
    # - ./FlashLayout/SubPartitions/EsePackage/PunitSubPartition/num_of_instance_PUNITBinary_path
    #   (Adding ./FlashLayout/SubPartitions/EsePackage/PunitSubPartition/PUNITBinary<number> XML tag)
    xml_multi_instanced_element = {
        './FlashLayout/SubPartitions/EsePackage/DmuSubPartition/num_of_instance_DMUBinary_path': {
            'path': './FlashLayout/SubPartitions/EsePackage/DmuSubPartition/DMUBinary{}',
            'value': '',
            'label': 'DMU Section {} Binary',
            'help_text': 'This loads the DMU Section binary that will be merged into the output image generated by the Intel(R) FIT tool.',
            'key': "ESEPlugin:DMUP:DMUBinary_path{}"
        },
        './FlashLayout/SubPartitions/EsePackage/PunitSubPartition/num_of_instance_PUNITBinary_path': {
            'path': './FlashLayout/SubPartitions/EsePackage/PunitSubPartition/PUNITBinary{}',
            'value': '',
            'label': 'PUNIT Section {} Binary',
            'help_text': 'This loads the PUNIT Section binary that will be merged into the output image generated by the Intel(R) FIT tool.',
            'key': "ESEPlugin:PUNIT:PUNITBinary_path{}"
        }
    }

    for num_of_instance_tag, new_element_attr in xml_multi_instanced_element.items():
        num_of_instances = None
        for each in xml_change_list:
            for xml_path, xml_value in each:
                if xml_path in num_of_instance_tag:
                    if not xml_value.isdigit():
                        raise Exception("{} has {} value. Expected a numerical value".format(xml_path, xml_value))
                    num_of_instances = int(xml_value)
                    break
            if num_of_instances != None:
                break

        if num_of_instances >= 2:
            new_element_path = new_element_attr.pop('path')
            for num in range(2, num_of_instances + 1):
                full_element_path = new_element_path.format(num)
                if not root.find(new_element_path):
                    full_element_path = os.path.split(full_element_path)
                    new_path = full_element_path[0]
                    new_tag = full_element_path[1]

                    new_path = root.find(new_path)
                    new_element = ET.Element(new_tag)
                    for attr, value in new_element_attr.items():
                        new_element.attrib[attr] = value.format(num)
...
def stitch (stitch_dir, stitch_cfg_file, sbl_file, btg_profile, plt_params_list, platform_data, platform, tpm, full_rdundant = True):
    ...
    # TODO: Modify or remove patch_xml_file function if not needed.
    # ./FlashSettings/BiosConfiguration/TopSwapOverride no longer exist in ARL-S IFWI, and seems to be replaced by
    # ./FlashSettings/BiosConfiguration/PchTopSwapOverride and ./FlashSettings/BiosConfiguration/SocTopSwapOverride
