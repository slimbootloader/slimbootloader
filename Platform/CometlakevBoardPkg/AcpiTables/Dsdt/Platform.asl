...
// This is not applicable for EVCRB board.
#ifndef CMLV_FLAG
  If(LOr(LEqual(Arg0,3), LEqual(Arg0,4)))
  {
    ...
    //
    If(LAnd(UTCE,LEqual(\_SB.GGIV(GPIO_CNL_H_GPP_G7), 0x00)))
    {

      // Pull GPIO down to notify PD controller
      \_SB.SGOV(GPIO_CNL_H_GPP_G1, 0)
      Sleep(150)
      ...
      While(LLess(Local0, 10))
      {
        ...
        If (LEqual(\_SB.GGOV(GPIO_CNL_H_GPP_G6), 1))
        {
          Break
        } Else {
          // Time out in 1 sec
  ...
  // StandBy LED Drive. Program High for sleep state s3, s4 and s5 only for desktop.
  If(LOr(LOr(LEqual(Arg0,3), LEqual(Arg0,4)), LEqual(Arg0,5))){
    If(LEqual(PFLV,FlavorDesktop)){
#ifndef CMLV_FLAG
      \_SB.SGOV(GPIO_CNL_LP_GPP_H23,1)
#else
      // This is not applicable for EVCRB (PCH - H) board.
#endif
    }
  }
}
...
// This is not applicable for EVCRB board.
    ...
    If(LAnd(UTCE,LEqual(\_SB.GGIV(GPIO_CNL_H_GPP_G7), 0x00)))
    {
      ...
      While(LLess(Local0, 10))
      {
        ...
        If (LEqual(\_SB.GGOV(GPIO_CNL_H_GPP_G6), 0))
        {
          Break
        } Else {
          // Time out in 1 sec
