## @ StitchIfwi.py
#  This is a python stitching script for Slim Bootloader KSV build
#
...
from   security_stitch_help_ksv  import *
...
def gen_xml_file(stitch_dir, stitch_cfg_file, btg_profile, platform, plt_params_list, tpm):
    ...
    xml_change_list, xml_remove_list, xml_insert_list = stitch_cfg_file.get_xml_change_list (platform, plt_params_list, btg_profile)

    # For updating attributes
    for xml_path, attribute, value in xml_change_list:
        ...
        if node != None:
            ...
            print (xml_path, attribute, value)
            node.set(attribute, value)
        else:
            print("Warning: ", xml_path, "NOT found")
        print("\n")

    # For add new entry
    for container in xml_insert_list.keys():
        container_node = root.find('%s' % container)
        for setting in xml_insert_list[container]:
            new_elemenet = ET.fromstring(setting)
            new_elemenet.tail = "\n    "
            container_node.append(new_elemenet)

    # For remove exsisting entry
    for container in xml_remove_list.keys():
        container_node = root.find('%s' % container)
        for setting in xml_remove_list[container]:
            setting_node = container_node.find('%s' % setting)
            container_node.remove(setting_node)
...
def stitch (stitch_dir, stitch_cfg_file, sbl_file, btg_profile, plt_params_list, platform_data, platform, tpm, full_rdundant = True):
    ...
    # Generate xml
    gen_xml_file(stitch_dir, stitch_cfg_file, btg_profile, platform, plt_params_list, tpm)
...
def main():
    ...
    ap.add_argument('-p', dest='platform', choices = ['ksv'], default = 'ksv', help='(optional) specify platform sku to stitch')
    ...
    ap.add_argument('-t', dest='tpm', default = 'dtpm', choices=['dtpm', 'none'], help='specify TPM type')
    ...
    ap.add_argument('-op', dest='outpath', default = '', help="Specify path to write output IFWI and signed binary files")
    ...
    generated_ifwi_file = os.path.join(work_dir, 'Fit', 'outimage.bin')
