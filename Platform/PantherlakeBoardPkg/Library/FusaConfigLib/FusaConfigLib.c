  ...
  )
{
  ...
  if (FusaCfgData != NULL) {
    ...
    FspmConfig->Module0LockstepSupport     = 0x1;
    FspmConfig->Module1LockstepSupport     = 0x1;
    FspmConfig->LpcLockstepSupport         = 0x1;
  }
  ...
  )
{
  ...
  FspsConfig->CpuCrashLogEnable          = FusaCfgData->CpuCrashLogEnable;
  ...
  if (SiCfgData != NULL) {
    FspsConfig->EnableRp = (SiCfgData->L2CatEnable == 1) ? 0 : 0x1;
    DEBUG((DEBUG_INFO, "L2 QOS Enum is %s\n",(SiCfgData->L2CatEnable == 1) ? L"Enabled" : L"Disabled"));
  } else {
    DEBUG((DEBUG_ERROR, "Silicon Config Data not found!\n"));
  }

  // Load FuSa binary ingredients
  // This is the last sequence so fail out if any step fails.
  ...
  Status = RegisterContainer((UINT32)(UINTN)FusaCompInMem, FusaCompLength, NULL);
  ...
  if (EFI_ERROR(Status)) {
    DEBUG((DEBUG_ERROR, "Failed to load FuSa Startup BIST pattern! %r\n", Status));
    return Status;
  }
