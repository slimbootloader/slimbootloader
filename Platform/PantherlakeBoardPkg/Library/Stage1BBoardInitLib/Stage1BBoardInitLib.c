...
DEBUG_CODE_END();
  ...
  case PostConfigInit:
    ...
    FeaturesCfgData = (FEATURES_CFG_DATA *) FindConfigDataByTag (CDATA_FEATURES_TAG);
    if (FeaturesCfgData != NULL) {
      if (FeaturesCfgData->Features.Sio == 1) {
        EclessSioInit();
      } else {
        DEBUG ((DEBUG_INFO, "S0ix enabled: %x .. Skipping EclessSioInit\n", FeaturesCfgData->Features.S0ix));
      }
    }
  ...
  case PreMemoryInit:
    ...
    if (GetPlatformId () == PLATFORM_ID_PTL_UH_DDR5_EMB_CRB) {
      // I2c post code initialization
      MaxLedInit ();
    }
    break;
  case PostMemoryInit:
