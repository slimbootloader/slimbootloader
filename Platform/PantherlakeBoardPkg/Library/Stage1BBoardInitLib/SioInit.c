...
)
{
  ...
  DEBUG((DEBUG_INFO, "EclessSioInit - LpcIoDecodeRanges: %x\n", LpcIoDecodeRanges));
  ...
  if ((IoRead8(SIO_DATA_PORT) & 0xFF) == 0x86)
  {
    ...
    {
      DEBUG((DEBUG_INFO, "EclessSioInit - Return CHIPID2:Not ITE8659\n"));
      return;
    }
  }
  else
  {
    DEBUG((DEBUG_INFO, "EclessSioInit - Return CHIPID1:Not ITE8659\n"));
    return;
  }

  for (Index = 0; Index < sizeof(mSioIt8659TableSerialPort) / sizeof(EFI_SIO_TABLE); Index++) {
    DEBUG((DEBUG_INFO, "EclessSioInit - Reg:%x Val:%x\n", mSioIt8659TableSerialPort[Index].Register, mSioIt8659TableSerialPort[Index].Value));
  ...
  LpcIoDecodeRanges = PciSegmentRead16(LpcBaseAddr + (0x80));
  DEBUG((DEBUG_INFO, "EclessSioInit - R_LPC_CFG_IOD LpcIoDecodeRanges: %x\n", LpcIoDecodeRanges));

  Init16550Uart();
  DEBUG((DEBUG_INFO, "EclessSioInit - UARTINIT Done\n"));
