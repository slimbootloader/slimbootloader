...
/**
  Read stitched platform data and use PlatformId when available.

  @param[in, out] PlatformId   PlatformId to update.
**/
VOID
GetPlatformIdFromStitchData (
  IN OUT UINT8                *PlatformId
  )
{
  STITCH_DATA   *StitchData;

  StitchData = (STITCH_DATA *)(UINTN)(0xFFFC0040);
  if ((StitchData->Marker == 0xAA) && (StitchData->PlatformId > 0) && (StitchData->PlatformId < 32)) {
    *PlatformId = StitchData->PlatformId;
    DEBUG ((DEBUG_INFO, "Using stitched PlatformId 0x%X\n", *PlatformId));
  }
  else{
    DEBUG((DEBUG_INFO, "Did not get platformid from stitchdata\n"));
  }
}
  ...
  )
{
  ...
  if ((Status0 == EFI_SUCCESS) && (Status1 == EFI_SUCCESS)) {
    BoardId = (UINT8) SmbusInputPort1Info.InputPort1Fields.BoardId;
    ...
    DEBUG ((DEBUG_INFO, "BoardId from Smbus Io expander is 0x%x\n", BoardId));
  ...
  switch (BoardId) {
    case BoardIdPtlUHLp5Rvp1:
      *PlatformId = PLATFORM_ID_PTL_UH_LP5X_T3_RVP;
      break;
    case BoardIdPtlUHLp5Robinson:
      *PlatformId = PLATFORM_ID_PTL_UH_LP5X_Robinson;
      break;
    case BoardIdPtlUHLp5T4MemSktRvp:
      *PlatformId = PLATFORM_ID_PTL_UH_LP5X_T4_RVP;
      break;
    case BoardIdPtlUHDdr5Rvp4:
      *PlatformId = PLATFORM_ID_PTL_UH_DDR5_T3_RVP;
      break;
    case BoardIdPtlUHDdr5EmbCrb:
      *PlatformId = PLATFORM_ID_PTL_UH_DDR5_EMB_CRB;
      break;
    case BoardIdWclDdr5Crb:
      *PlatformId = PLATFORM_ID_WCL_DDR5_CRB;
      break;
    case BoardIdWclLp5Rvp:
      *PlatformId = PLATFORM_ID_WCL_LP5X_RVP;
      break;
    case BoardIdWclDdr5Rvp:
      *PlatformId = PLATFORM_ID_WCL_DDR5_RVP;
      break;
    default:
       DEBUG((DEBUG_ERROR, "Unsupported Board Id\n"));
    break;
  }
}
  ...
  )
{
  ...
  *BoardId = (UINT8) BoardInfo.RvpFields.BoardId;
}
...
)
{
  ...
  // below.
  //
  *PlatformId = PLATFORM_ID_UNKNOWN_RVP;
  ...
  if (BoardID == 0xFF){
    ...
    if (*PlatformId == 0xFF) {
      DEBUG ((DEBUG_INFO, "Calling GetPLatformIdFromStitchData()\n"));
      GetPlatformIdFromStitchData (PlatformId);
    }
    return;
  }
