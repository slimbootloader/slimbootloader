...
External(EDMX)
External(\_SB.PARENT_OF_LPCB.LPCB.H_EC.LID0, DeviceObj)
External(\_SB.PARENT_OF_LPCB.LPCB.H_EC.UPLD, MethodObj)
External(\_SB.PARENT_OF_LPCB.LPCB.H_EC.RLDS, MethodObj)
//
    ...
    Switch(ToInteger(Arg2)) {
      ...
      Case (0)
      {
        If (LEqual(Arg1, 1)) { // test Arg1 for Revision ID: 1
          Store ("IGPU Supported Functions Bitmap ", Debug)
      ...
      //
      // Function Index: 21
      // Get and Set Display Mux GPIO control for IGPU
      // Arg3 : 0 to get mux status
      //        1 to set mux control to IGPU
      ...
      Case(22) {
        Switch (ToInteger(Arg3)) {
          Case(0)
            {
              If (CondRefOf(\_SB.PARENT_OF_LPCB.LPCB.H_EC)) {
                \_SB.PARENT_OF_LPCB.LPCB.H_EC.UPLD (0)
                Store(0x0, LIDS)
                Store(0, \_SB.PC00.GFX0.CLID)
                Notify(\_SB.PARENT_OF_LPCB.LPCB.H_EC.LID0, 0x80)
                Return(0)
              }
            }
          ...
          Case(1)
            {
              If (CondRefOf (\_SB.PARENT_OF_LPCB.LPCB.H_EC)) {
                \_SB.PARENT_OF_LPCB.LPCB.H_EC.UPLD (1)
                Store(0x1, LIDS)
