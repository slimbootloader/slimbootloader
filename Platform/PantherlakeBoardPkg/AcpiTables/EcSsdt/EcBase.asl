...
// ----------------------------------------------------------
// Disable ACPI Burst mode for byte length less than 2 bytes
// ----------------------------------------------------------
...
{
  If (LEqual (Arg0, ToUUID ("ecc0d5e9-3ee7-4f53-8c8f-766b839dddce"))) // UUID for ACPI Burst mode
  {
    ADBG ("ACPI Busrt Mode _DSM")
    Switch (ToInteger (Arg2)) {
      //
      // Function 0: Return supported functions based on the revision
      //
      Case (0)
      {
        ...
        Return (Buffer () {0x0}) // revision 1+: No function yet supported
      }

      //
      // Function 1: Return platform-FW granted min number of bytes to use busrt mode for
      //
      Case (1)
      {
        //
        // Platform-FW logic to get min number of bytes to use busrt mode for, it is
        // recommended to return a value larger than 1
        ADBG ("Return a value larger than 1 to enable ACPI Burst mode")
        Return (Buffer () {0x2})
      }
    } // End switch
  } // End UUID check for ACPI Burst Mode
  Return (Buffer () {0})
}
...
//
Name (VBDS, 0)
Name (ONTM, 0) // Temporary variable to initialize VBDS only once in the boot
...
Method (EREG)
{
  ...
  ADBG (Concatenate ("BNUM battery state: ", ToHexString (BNUM)))
  ...
  ADBG (Concatenate ("Checking PWRS: ", ToHexString (PWRS)))

  And (ECRD (RefOf (PWRT)), 0x07, Local0)
  ADBG (Concatenate ("Get PWRT (EC Ram Offset 0x4E) low 3 bits: ", ToHexString (Local0)))
...
Method (_INI)
{
  ...
  If (LEqual (\S0ID, 1)) {
    ...
    ECMD (EC_C_BTN_SCI_CTL)                                  // Enable/Disable SCIs from buttons
  }
}
