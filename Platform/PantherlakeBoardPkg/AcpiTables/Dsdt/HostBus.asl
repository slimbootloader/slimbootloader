...
Method (GMHB,0,Serialized)
{
  if (LEqual (MH_B,0))
  {
    ShiftLeft (\_SB.PC00.MC.MHBR,18,MH_B)
  }
  Return (MH_B)
}
...
//
Method (PC2M, 2, Serialized) {      // Convert _ADR to PCI MMIO address.
  Store (\_SB.PC00.GPCB (), Local0) // MMIO Base address
  Add (Local0, ShiftLeft (Arg1, 20), Local0) //Bus no [27:20]
...
Method (_CRS,0,Serialized)
{
  ...
  //
  // Memory range fixup for PCH-S.
  //
  If (LNotEqual (PCHA, 0)) {
    // SoC bus ranges
    Store (SBSB, PBMN)
    Store (SBSL, PBMX)
    Subtract (SBSL, SBSB, PBLN)
    Add (PBLN, 1, PBLN)
...
Scope (\_SB.PC00)
{
  Method (AR00) {
    ...
    If (LOr (LEqual (PCHS, PCHP), LEqual (PCHS, PCHW))) {
      Return (PAR0) // APIC mode buffer for PCD P and WCL PCD
#else
    If (LEqual (PCHS, PCHP)) {
      Return (PAR0) // APIC mode buffer for PCD P
#endif
  ...
  //
  // Device NPU (0/11/0) is a PCI device
  //
  Device (NPU0) {
