...
External (UPRU, MethodObj)
  ...
  Method (_DSM,4,serialized) {
    ...
    If (LEqual (Arg0, ToUUID ("203B203B-1F13-409A-9AAE-1A0347525584"))) {
      Switch (ToInteger (Arg2)) {
        ...
        Case (1) {
          // Function 1: Return true if HSII workaround is needed to be applied and clear HSII.
          If (CondRefOf (AHWA)) {
            Return (LAnd (AHWA, 1))
          } Else {
            Return (0)
  ...
  Device (RHUB)
  {
    ...
    //
    // PCH XHCI controller _DSM method
    //
    Method (_DSM,4,serialized)
    {
      If(LEqual(Arg0, ToUUID("ce2ee385-00e6-48cb-9f05-2edb927c4899"))) {
        ...
        If (LEqual (Arg2, 0)) {
          ADBG("[PCH] XHCI RHUB F0")
          Return (Buffer () {0x81})
        } // function 0: list of supported functions
        If (LEqual (Arg2, 7)) {
          ADBG("[PCH] XHCI RHUB F7")
          Return (0x01)
