...
**/
  ...
  External (PCPR, MethodObj) // PCIe slot core power on/off method
  ...
  External (ADSD, MethodObj) // Alternate DSD object
  External (HD3I, MethodObj) // Pcie RP RTD3 specific initialization Hook.
  External (PRWH, MethodObj) // Pcie RP RTD3 specific _PRW Hook.
  External (PS0H, MethodObj) // PCIe root port RTD3 _PS0 Hook Function.
  ...
  Field (PXCS,AnyAcc, NoLock, Preserve)
  {
    ...
    , 24,
    LTSM, 8,
    Offset (R_PCH_PCIE_CFG_LTROVR2),   // 0x404, LTR Override
    LSOE, 1,                           // 0,   B_PCIE_CFG_LTROVR2_LTRSOVREN
    LNSE, 1,                           // 1,   B_PCIE_CFG_LTROVR2_LTRNSOVREN
  }
  ...
  Method (DL23, 0, Serialized) {
    ...
    If (LAnd (LEqual(LASX, 1), LEqual(LNRE, 0))) {
      If (LOr(LEqual(LSOE, 1), LEqual(LNSE, 1))) {
        Store (1, LNRE)
        Sleep (1)
        Store (0, LNRE)
        ADBG ("Toggle LNRE")
  ...
  Method (_DSM, 4, Serialized) {
    ...
    If (LEqual (Arg0, ToUUID ("E5C937D0-3553-4d7a-9117-EA4D19C3434D"))) {
      ...
      // Arg1: Revision ID: 2
      // Arg2: Function Index: 0, 6, 8, 9, 10, 11
      // Arg3: Empty Package
      //
      // Return:
      ...
      Switch (ToInteger (Arg2)) {
        ...
        Case (10) {
          Store (0, Local0)
          If (CondRefOf (PCPR)) {
            Store (PCPR (Arg3), Local0)
          }
          If (CondRefOf (PPBA)) {
            Store (PPBA (Arg3), Local0)
          }
          Return (Local0)
        }
        //
  ...
  Device (PXSX)
  {
    ...
    Method (_PRW, 0) {
      ADBG (Concatenate ("_PRW PCIe RP PXSX", ToHexString (SLOT)))
      If (CondRefOf (GP1E)) {
        If (LNotEqual (GP1E, 0)) {
          Return (GPRW (PCIE_HP_EN, 4))
        }
      }
  ...
  Method (_PRW, 0) {
    ...
    Store (0x69, Local0) // Legacy PCI_EXP_STS GPE Event
    If (CondRefOf (GP1E)) {
      If (LNotEqual (GP1E, 0)) {
        Store (PCIE_HP_EN, Local0)
      }
    }
    If (CondRefOf (PRWH)) {
      Return (PRWH (Local0))
    } Else {
      Return (GPRW (Local0, 4))
    }
  }
  ...
  Method (_PS0, 0, Serialized)
  {
    ...
    If (CondRefOf (PS0H)) {
      ADBG ("Calling PCIE RP RTD3 PS0 Hook")
      PS0H ()
      ADBG ("Returned from PCIE RP RTD3 PS0 Hook")
    }
    If (CondRefOf (PPS0)) {
  ...
  Method (_PS3, 0, Serialized)
  {
    ...
    If (CondRefOf (PS3H)) {
      ADBG ("Calling PCIE RP RTD3 PS3 Hook")
      PS3H ()
      ADBG ("Returned from PCIE RP RTD3 PS3 Hook")
    }
    If (CondRefOf (PPS3)) {
