...
Method (GGPO, 0x1, Serialized)
{
  ...
  // Local0 - GPIO pad index
  // Local1 - GPIO com index
  // Local2 - GpioCommunityAddress
  // Local3 - 16 upper bits of RVID
  // Local4 - PadOwnership register address
  // Local5 - return value
  Store (GNMB (Arg0), Local0)
  Store (GCOM (Arg0), Local1)
  Store (Add (GCMP (Local1), SBRG), Local2)

  // RVID address offset in each community is 0x00
  OperationRegion (RREG, SystemMemory, Local2, 4)
  Field (RREG, AnyAcc, NoLock, Preserve)
  {
    RVID,32
  }

  // Local3 - RVID[31:16] - upper 16 bits of RVID (Revision ID) register of a GPIO community
  Store (ShiftRight(RVID, 16), Local3)

  If (LGreaterEqual(Local3, GPIO_HW_BASE_REVID_HAS_1_1)) {
    // Register layout compliant with HAS_1.1 and above
    // PadOwnership register address = (PadOwnership address of Gpio Pad 0 in this group) + (GPIO Pad Index) * 4
    Store (Add (GET_PAD_OWN_REG_ADDRESS (Arg0), Multiply (Local0, 0x4)), Local4)
  } Else {
    // Register layout compliant with HAS_1.0 and below
    // PadOwnership register address = (PadOwnership address of Gpio Pad 0 in this group) + (GPIO Pad Index >> 3) * 4
    Store (Add (GET_PAD_OWN_REG_ADDRESS (Arg0), Multiply (ShiftRight (Local0, 3), 0x4)), Local4)
  }

  OperationRegion (PORG, SystemMemory, Local4, 4)
  Field (PORG, AnyAcc, NoLock, Preserve)
  {
    TEMP,32
  }
  ...
  If (LGreaterEqual (Local3, GPIO_HW_BASE_REVID_HAS_1_1)) {
    // Register layout compliant with HAS_1.1 and above
    // PadOwnValue = PadOwnRegisterValue & 0x07
    Store (And (TEMP, 0x07), Local5)
  } Else {
    // Register layout compliant with HAS_1.0 and below
    // PadOwnValue = (PadOwnRegisterValue >> (((Gpio Pad Index) & 0x7) * 4)) & 0x3
    Store (And (ShiftRight (TEMP, Multiply (And (Local0, 0x7), 0x4)), 0x3), Local5)
  }
