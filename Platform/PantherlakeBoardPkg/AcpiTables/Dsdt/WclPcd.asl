  ...
  Device (PRRE) {
    ...
    Method (_CRS,0,Serialized)
    {
      Name (BUF0,ResourceTemplate () {
        ...
        // Refer to Defines/PchReservedResources.h for ranges used by PCH
        //
      ...
      //
      // WCL_PCD_GPIOV2_COM0 = 0x59
      // WCL_PCD_GPIOV2_COM1 = 0x5A
      // WCL_PCD_GPIOV2_COM3 = 0x5B
      // WCL_PCD_GPIOV2_COM4 = 0x5C
      // WCL_PCD_GPIOV2_COM5 = 0x5D
      //
      ...
      Name (SBR0, ResourceTemplate () {
        ...
        CreateQWordField (SBR0, SBA0._MIN, BAS0)
        CreateQWordField (SBR0, SBA0._LEN, LEN0)
        CreateQWordField (SBR0, SBA0._MAX, LIM0)
        Store (SBRG, BAS0)
        Store (WCL_PCD_GPIOV2_COM0, LEN0)
        Store (Subtract (Add (BAS0, LEN0), 1), LIM0)
        ConcatenateResTemplate (BUF0, SBR0, Local0)

      Name (SBR1, ResourceTemplate () {
        QWordMemory (
              ResourceConsumer, ,
              MinNotFixed,
              MaxNotFixed,
      ...
      CreateQWordField (SBR0, SBA1._MIN, BAS1)
      CreateQWordField (SBR0, SBA1._LEN, LEN1)
      CreateQWordField (SBR0, SBA1._MAX, LIM1)
      Store (Add (Add (SBRG, WCL_PCD_GPIOV2_COM5), PCR_MMIO_SIZE_PER_PID), BAS1)
      Store (Subtract (Add (SBRG, PCR_MMIO_SIZE_EX), BAS1), LEN1)
      Store (Subtract (Add (SBRG, PCR_MMIO_SIZE_EX), 1), LIM1)
      ConcatenateResTemplate (Local0, SBR1, Local1)

      //
      // claim Second Side band address.
      //
      Name (SBR2, ResourceTemplate () {
        QWordMemory (
              ResourceConsumer, ,
              MinNotFixed,
              MaxNotFixed,
      ...
      CreateQWordField (SBR0, SBA2._MIN, BAS2)
      CreateQWordField (SBR0, SBA2._LEN, LEN2)
      CreateQWordField (SBR0, SBA2._MAX, LIM2)
      Store (PCH_PCR_SECOND_BASE_ADDRESS, BAS2)
      Store (PCR_MMIO_SIZE_EX, LEN2)
      Store (Subtract (Add (PCH_PCR_SECOND_BASE_ADDRESS, PCR_MMIO_SIZE_EX), 1), LIM2)
      ConcatenateResTemplate (Local1, SBR2, Local2)
...
Scope (\_SB_.PC00) {
  ...
  // GbE Controller
  //
  include ("PtlPcdGbe.asl")

  //
  // xHCI Controller - Device 20, Function 0
  //
  Include ("PtlPcdXhci.asl")

  //
  // HD-Audio
  //
  Include ("PtlPcdHda.asl")
}

Include ("PchFruWclPcd.asl")
