...
External(WADT_ACPI_DEV_PATH_ACWA)
External(WADT_ACPI_DEV_PATH_DCWA)
External(WADT_ACPI_DEV_PATH_ACET)
External(WADT_ACPI_DEV_PATH_DCET)
...
Device (AWAC) { // Time and Wake Alarm Device
  ...
  Method (_GWS, 1, Serialized) { // Get Wake Alarm Status
    ...
    If (LEqual (Arg0, 0)) { // AC timer
      If (LAnd (LEqual (WADT_ACPI_DEV_PATH_ACWA, 0xFFFFFFFF), And (BIT0, WTTR))) { // AC Timer expired.
        Or (BIT0, Local0, Local0)
        Xor (BIT0, WTTR, WTTR) // Clear AC timer tracker
      }
    }
    ElseIf (LAnd (LEqual (WADT_ACPI_DEV_PATH_DCWA, 0xFFFFFFFF),And (BIT1, WTTR))) { // DC Timer expired.
      Or (BIT0, Local0, Local0)
  ...
  Method (_STP, 2) { // Set Expired Timer Wake Policy
    ...
    // Return 0 - Success, 1 - Failure
    If (LEqual (Arg0, 0)) { // AC Timer
      Store (Arg1, WADT_ACPI_DEV_PATH_ACET)
    } Else { // DC Timer
      Store (Arg1, WADT_ACPI_DEV_PATH_DCET)
    }
    Return (0) // Always success
  }
  Method (_STV, 2, Serialized) { // Set Timer Value
    ...
    // Return 0 - Success, 1 - Failure
    If (LEqual (Arg0, 0)) {      // AC Timer
      Store (Arg1, WADT_ACPI_DEV_PATH_ACWA)
      Or (BIT0, WTTR, WTTR)
    } Else {   // DC Timer
      Store (Arg1, WADT_ACPI_DEV_PATH_DCWA)
      Or (BIT1, WTTR, WTTR)
    }
    Return (0) // Success
  }
  Method (_TIP, 1) { // Expired Timer Wake Policy
    ...
    If (LEqual (Arg0, 0)) { // AC Timer Wake Policy
      Return (WADT_ACPI_DEV_PATH_ACET)
    } Else {                // DC Timer Wake Policy
      Return (WADT_ACPI_DEV_PATH_DCET)
    }
  }
  Method (_TIV, 1) { // Timer Values
    ...
    If (LEqual (Arg0, 0)) { // AC Timer
      Return (WADT_ACPI_DEV_PATH_ACWA)
    } Else {                // DC Timer
      Return (WADT_ACPI_DEV_PATH_DCWA)
    }
  }
} // Device (AWAC)
