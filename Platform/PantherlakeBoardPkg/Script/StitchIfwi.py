...
from   security_stitch_help_ptl  import *
...
def gen_xml_file(stitch_dir, stitch_cfg_file, btg_profile, plt_params_list, platform, tpm):
    ...
    temp_xml_file = os.path.join (stitch_dir, 'Temp', 'temp.xml')
...
def stitch (stitch_dir, stitch_cfg_file, sbl_file, btg_profile, plt_params_list, platform_data, platform, tpm, full_rdundant = True):
    ...
    # Generate xml
    if platform == 'ptl' or platform == 'ptla' or platform == 'wcl':
    ...
    #shutil.copy(os.path.join(temp_dir,'Bsis.bin', os.path.join(temp_dir, "..",BpmGen))
...
def main():
    ...
    if 'tsn' in plt_params_list or 'fusa' in plt_params_list or 'posc' in plt_params_list:
      if args.key_dir:
