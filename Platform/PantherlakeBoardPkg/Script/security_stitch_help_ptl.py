## @ security_stitch_help_ptl.py
#  This is a python stitching script for security helper functions and xml updates
#
...
def update_tpm_type(tpm_type, tree):
  ...
  ptt_supported_xml_entry     = "./PlatformProtection/Tpms/TPMTechnology"
  ptt_supported_fpf_xml_entry = "./PlatformProtection/Tpms/IntelPttConfiguration/PttSupportedFpf"
  dtpm_supported_fpf_xml_entry = "./PlatformProtection/Tpms/DtpmConfiguration/dTpmSupportedFpf"

  tpm_config = {
    'ptt': {
              ptt_supported_xml_entry: 'Intel (R) PTT',
              ptt_supported_fpf_xml_entry: 'Yes',
              dtpm_supported_fpf_xml_entry: 'No',
            },
    'dtpm': {
              ptt_supported_xml_entry: 'dTPM',
              ptt_supported_fpf_xml_entry: 'No',
              dtpm_supported_fpf_xml_entry: 'Yes',
            },
    'none': {
              ptt_supported_xml_entry: 'Intel (R) PTT',
...
def gen_bpmgen2_params (stitch_cfg_file, InFile, OutFile):
    ...
    for item in params_change_list:
        for variable, value in item:
            if '/' in variable:
                # Section-scoped override: 'SECTION/Key' targets Key under '# SECTION'
