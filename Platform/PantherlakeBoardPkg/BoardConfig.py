...
class Board(BaseBoard):
    def __init__(self, *args, **kwargs):
        ...
        self.FSP_IMAGE_ID         = '$PTLFSPE'
        self.HAVE_FSP_AT_RESET    = False
        self.HAVE_SIGNED_FSP      = False
        self.BOARD_PKG_NAME_OVERRIDE = 'PantherlakeBoardPkg'
        self._EXTRA_INC_PATH      = ['Silicon/PantherlakePkg/Ptl/Fsp']
        self._FSP_PATH_NAME       = 'Silicon/PantherlakePkg/Ptl/Fsp'
        self._SMBIOS_YAML_FILE    = os.path.join('Platform', self.BOARD_PKG_NAME, 'SmbiosStrings.yaml')
        self.FSP_INF_FILE         = 'Silicon/PantherlakePkg/Ptl/Fsp/FspBin.inf'
        self.MICROCODE_INF_FILE   = 'Silicon/PantherlakePkg/Ptl/Microcode/Microcode.inf'
        ...
        # TCC on PTL doesn't introduce any build changes except a small amount of init code.
        ...
        if self.FUSA_SUPPORT:
            self.LOADER_RSVD_MEM_SIZE += 0x8800000 # reserve 136MB for CrashLog use if enabled, can be reduced if needed
            ...
            # Allow TCC be enabled by FuSa dlt file
            FusaConfig = ''
        ...
        #   VbtBin folder.
        self._MULTI_VBT_FILE      = {1:'Vbt.dat', 2:'VbtTc3Hdmi.dat', 3: 'VbtRobinson.dat'}

        self.CFG_DATABASE_SIZE    = self.CFGDATA_SIZE
        self._generated_cfg_file_prefix = 'Autogen_'
        self._CFGDATA_DEF_FILE = 'CfgDataDef_Ptlp.yaml'
        self._CFGDATA_INT_FILE = []
        self._CFGDATA_EXT_FILE = [self._generated_cfg_file_prefix + 'CfgDataInt_Ptlp_LPDDR5_Rvp1.dlt',\
                                  self._generated_cfg_file_prefix + 'CfgDataInt_Ptlp_LPDDR5_Rvp3.dlt',\
                                  self._generated_cfg_file_prefix + 'CfgDataInt_Ptlp_DDR5_Rvp.dlt',\
                                  self._generated_cfg_file_prefix + 'CfgDataInt_Ptlp_DDR5_Crb.dlt',\
                                  self._generated_cfg_file_prefix + 'CfgDataInt_Ptlp_Robinson.dlt']
    ...
    def GetPlatformDsc (self, BuildPkgName = "BootLoaderCorePkg"):
        ...
        if self.FUSA_SUPPORT:
            dsc['LibraryClasses.%s' % self.BUILD_ARCH].append ('CrashLogLib|Silicon/$(SILICON_PKG_NAME)/Library/FusaCrashLogLib/FusaCrashLogLib.inf')
            dsc['LibraryClasses.%s' % self.BUILD_ARCH].append ('FusaConfigLib|Platform/$(BOARD_PKG_NAME)/Library/FusaConfigLib/FusaConfigLib.inf')
            dsc['LibraryClasses.%s' % self.BUILD_ARCH].append ('PchGspiLib|Silicon/$(SILICON_PKG_NAME)/Library/PchGspiLib/PchGspiLib.inf')
