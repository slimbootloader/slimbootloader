...
//
// TESTMENU:RestrictedBegin
//
#if FixedPcdGetBool(PcdTestMenuEnable) == 1
  If(LEqual(TPRP,1)) {
#endif

//
// TESTMENU:RestrictedEnd
//
  ...
  If(LEqual(XDCE,1)) {
    ...
    Scope(\_SB.PC00.XDCI)
    {
      ...
      Method (_PS3, 0, NotSerialized)
      {
        ...
        //ADBG("XDC CSD3") // ADBG:InternalOnlyContent
      ...
      Method (_PS0, 0, NotSerialized)
      {
        ...
        //ADBG("XDC CSD0") // ADBG:InternalOnlyContent
...
If (LNotEqual(GBES,0)) {
  Scope(\_SB.PC00.GLAN)
  {
    Method (_PS3, 0, NotSerialized)
    {
      //ADBG("GBE CSD3") // ADBG:InternalOnlyContent
      \_SB.CSD3(MODPHY_SPD_GATING_GBE)
    }
    Method (_PS0, 0, NotSerialized)
    {
      If(LNot(GBED)){  // If GBE_FDIS_PMC == 0
        //ADBG("GBE CSD0") // ADBG:InternalOnlyContent
...
//
    Scope(\_SB.PC00.I2C0){
      ...
      {
        //ADBG("I2C0 _PS0") // ADBG:InternalOnlyContent
      }
      /// PS3X Method, called by PS3 method in PchSerialIo.asl
      Method(PS3X,0,Serialized)
      {
        //ADBG("I2C0 _PS3") // ADBG:InternalOnlyContent
      }
      /// \ref i2c0_pr_pxtc
      ...
      Method(PSTA, 1, Serialized){
        If (Arg0 == 0) {
          //ADBG("I2C0 TPD _STA ON")  // ADBG:InternalOnlyContent
          Return(0x01)
        }
        If (Arg0 == 1) {
          If(LEqual(\_SB.GGOV(GPLP), 1)) {
            //ADBG("I2C0 TPL _STA ON")  // ADBG:InternalOnlyContent
            Return(0x01)
          } Else {
            //ADBG("I2C0 TPL _STA OFF") // ADBG:InternalOnlyContent
            Return(0x00)
          }
        }
        Return(0x00)
      ...
      // | Wacom                 |                0                      |                0                    |     //ACPI: RestrictedContent
      // |_______________________|_______________________________________|_____________________________________|     //ACPI: RestrictedContent
      // | Elan                  |                1                      |                2                    |     //ACPI: RestrictedContent
      // |_______________________|_______________________________________|_____________________________________|     //ACPI: RestrictedContent
      // | current setting       |                2                      |                3                    |
      // |_______________________|_______________________________________|_____________________________________|
      ...
      {
        If (Arg0 == 0) {
          //ADBG("I2C0 Tpd PWR ON") // ADBG:InternalOnlyContent
          // enable int line
          \_SB.SGRA(GPDI, PPDI)
        }
        ElseIf (Arg0 == 1) {
        //ADBG("I2C0 Touch PWR ON") // ADBG:InternalOnlyContent
        // drive pwr high
        \_SB.SGOV(GPLP, PPLP)
      ...
      {
        If (Arg0 == 0) {
          //ADBG("I2C0 Tpd PWR OFF") // ADBG:InternalOnlyContent
          // disable int line
          Xor(PPDI, 1, Local0)
          \_SB.SGRA(GPDI, Local0)
        }
        ElseIf (Arg0 == 1) {
        //ADBG("I2C0 Touch PWR OFF") // ADBG:InternalOnlyContent
          // disable int line
        Xor(PPLI, 1, Local0)
      ...
      If (LNotEqual(TPDT,0)) {
      Scope(TPD0){
        Name (TD_N, "TPD0")                                       // TD_N - Touch Device name string (for debug purpose) // ACPI:RestrictedContent
      ...
      If (LNotEqual(TPLT,0)) {
        Scope(TPL1){
          Name (TD_N, "TPL2")                                       // TD_N - Touch Device name string (for debug purpose) // ACPI:RestrictedContent
...
#if FixedPcdGetBool(PcdTestMenuEnable) == 1
//
// When HD AUDIO is enabled, AUDIO CODEC RTD3 should also be enabled
// But When HD AUDIO RTD3 is disabled then user should have the option to control AUDIO CODEC RTD3 alone through enable/disable mechanism
//
  If(LOr(LEqual(TACD,1), (LEqual(THAD,1)))) {
#endif
//
// TESTMENU:RestrictedEnd
//
    ...
    Scope(\_SB.PC00)
    {
      PowerResource(PAUD, 0, 0) {
        ...
        Method(_ON, 0){     /// _ON method \n
          //ADBG("Audio Codec LON") // ADBG:InternalOnlyContent
        ...
        Method(_OFF, 0){    /// _OFF method \n
          //ADBG("Audio Codec LOFF") // ADBG:InternalOnlyContent
        ...
        Method(PUAM, 0, Serialized)
        {
          ...
          If (LAnd(LEqual(^_STA, Zero), LNotEqual(\UAMS, Zero))) { ///New state = OFF Check if (_STA ==0 && \UAMS != 0) \n
            //ADBG("PAUD-PUAM OFF") // ADBG:InternalOnlyContent
          } Else { /// New state = ON (_STA=1) or (_STA=0 and \UAMS=0)
            /// Turn power on \n
            //ADBG("PAUD-PUAM ON") // ADBG:InternalOnlyContent
            If(LNotEqual(^PSTA, One)) { ///- Skip below if Power Resource is already in ON
              ...
              //ADBG("Audio Codec ON") // ADBG:InternalOnlyContent
            }
          }
...
//
    If(LNotEqual(\_SB.PC00.HDAS.VDID, 0xFFFFFFFF)) {
      Scope(\_SB.PC00.HDAS) {
        ...
        {
          //ADBG("HDAS.PS0X") // ADBG:InternalOnlyContent
...
//GPE Event handling - Start
  Scope(\_GPE) {
    ...
    //
    Method(AL6F) {
      //ADBG("AL6F Start") // ACPI:RestrictedContent
      // WLAN wake event
      If (\_SB.ISME(WLWK))
      {
        //ADBG("AL6F WLAN")   // ACPI:RestrictedContent
      ...
      If (\_SB.ISME(WWKP))
      {
        //ADBG("AL6F WWAN")   // ACPI:RestrictedContent
