...
ACPI_DEBUG_EXTERNAL_REFERENCE    // ADBG:RestrictedContent
...
Scope(\_SB)
{
  ...
  Device(IETM)
  {
      ...
      Method(_OSC, 4,Serialized,,BuffObj,{BuffObj,IntObj,IntObj,BuffObj})
      {
        ...
        //    STS0[0] = Reserved
        //    STS0[1] = _OSC Failure
        //    STS0[2] = Unrecognized UUID // IDSP is no longer required for Intel(R) Dynamic Tuning dynamic participants. // ACPI:InternalOnlyContent
                                          // Hence bios do not need to report Unrecognized UUID // ACPI:InternalOnlyContent
...
Scope(\_SB.PC00.LPCB.H_EC) // Open scope to Embedded Controller
{
  ...
  { // Thermal sensor threshold crossing event handler
    ADBG("Method _QF1")    // ADBG:RestrictedContent
