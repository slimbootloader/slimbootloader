...
Scope(\_SB)
{
  Device (PEPD)
  {
    ...
    Name(DEVY, Package() // uPEP Device List
    {
      ...
      Package() {"\\_SB.PC00.RP07.PXSX",          0x0, Package() {0, Package() {0xFF, 3}}},             // 43 - RP07
      ...
      Package() {"\\_SB.PC00.RP01",               0x0, Package() {0, Package() {0xFF, 3}}},                   // 89 - RP01
      Package() {"\\_SB.PC00.RP07",               0x0, Package() {0, Package() {0xFF, 3}}},                   // 90 - RP07
})
        ...
        If(LEqual(Arg2, One))
        {
          ...
          If(LEqual(And(PEPC, BIT4), BIT4)) // PEPC Bit[4] - En/Dis PCIe NVMe/AHCI
          {
            ...
            If(LOr(\_SB.PC00.RP01.PXSX.PAHC(), \_SB.PC00.RP01.PXSX.PNVM())){
              Store (0x01, Index (DeRefOf(Index (DEVY, 37)), 1)) // 37 - RP01.PXSX
