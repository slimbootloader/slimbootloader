...
// BEGIN OF ASL SCOPE
{
  ...
  External(\_SB.BAT2, DeviceObj)
  External(\_SB.IETM, DeviceObj)
  External(\_SB.SEN1, DeviceObj)
  External(\_SB.SEN2, DeviceObj)
  External(\_SB.SEN3, DeviceObj)
  External(\_SB.SEN4, DeviceObj)
  External(\_SB.SEN5, DeviceObj)
  ...
  Include ("EcLiteHidPlatformEventDev.asl")
  ...
  If (LAnd(LNotEqual(PSWP, 0), LEqual(RPNB, 0x01))) {
    Scope(\_SB.PC00.RP01) {
      Method (PPRW, 0) {
  ...
  If (LAnd(LNotEqual(PSW2, 0), LEqual(RPN2, 0x07))) {
    Scope(\_SB.PC00.RP07) {
      Method (PPRW, 0) {
