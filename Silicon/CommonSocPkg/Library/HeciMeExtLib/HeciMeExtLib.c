  ...
  )
{
  ...
  if (MeBaseAddress == 0) {
    return PciRead32 (PCI_LIB_ADDRESS(ME_BUS, ME_DEVICE_NUMBER, Function, Regoffset));
  }
  ...
  )
{
  ...
  MeBaseAddress = GetDeviceAddr (PlatformDeviceMe, 0);
  if (MeBaseAddress == 0) {
    return PciOr8 (PCI_LIB_ADDRESS(ME_BUS, ME_DEVICE_NUMBER, Function, Regoffset), OrValue);
  }
  Mebus         = (MeBaseAddress >> 16) & 0xFF;
  Medevice      = (MeBaseAddress >> 8) & 0xFF;
