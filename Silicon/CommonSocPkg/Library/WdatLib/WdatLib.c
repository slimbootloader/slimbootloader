  ...
  )
{
  ...
  Entry->RegisterRegion.Reserved          = AccessSize;
...
/**
  ...
  Note: WatchdogHeaderLength is decremented along with Header.Length to
  preserve byte-for-byte compatibility with the legacy in-place WDAT
  patcher in existing platforms. Per the Microsoft WDAT spec,
  WatchdogHeaderLength describes only the WDAT header (excluding
  instruction entries) and should be a fixed value, but legacy code
  decrements it on entry removal; this helper keeps that behavior.
**/
EFI_STATUS
EFIAPI
  ...
  )
{
  ...
  Wdat->NumberWatchdogInstructionEntries--;
  Wdat->WatchdogHeaderLength -= EntrySize;
