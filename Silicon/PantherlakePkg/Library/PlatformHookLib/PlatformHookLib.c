  ...
  )
{
  ...
  if (DebugPort >= PCH_MAX_SERIALIO_UART_CONTROLLERS) {
    ...
    PciAddress = mUartMmPciOffset[DebugPort] + (UINTN)PcdGet64(PcdPciExpressBaseAddress);
    MmioWrite32 (PciAddress + R_SERIAL_IO_CFG_BAR,  BarAddress);
    MmioWrite32 (PciAddress + R_SERIAL_IO_CFG_BAR_HIGH, 0x0);
    MmioWrite32 (PciAddress + R_SERIAL_IO_CFG_BAR1,  BarAddress + 0x1000);
