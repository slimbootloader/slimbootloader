  ...
  )
{
  ...
  switch (Private->AccessMethod) {
    case P2SbMmioAccess:
      ...
      ASSERT (PCH_PCR_BASE_ADDRESS);
      return MmioRead8 (P2SB_PCR_ADDRESS (PCH_PCR_BASE_ADDRESS, Private->P2SbPid, Offset));
    case P2SbMsgAccess:
      //
  ...
  )
{
  ...
  switch (Private->AccessMethod) {
    case P2SbMmioAccess:
      Offset = Offset + Private->Offset;
      ASSERT (PCH_PCR_BASE_ADDRESS);
      MmioWrite8 (P2SB_PCR_ADDRESS (PCH_PCR_BASE_ADDRESS, Private->P2SbPid, Offset), Value);
      //
  ...
  )
{
  ...
  switch (Private->AccessMethod) {
    case P2SbMmioAccess:
      Offset = Offset + Private->Offset;
      ASSERT (PCH_PCR_BASE_ADDRESS);
      return MmioRead16 (P2SB_PCR_ADDRESS (PCH_PCR_BASE_ADDRESS, Private->P2SbPid, Offset));
    case P2SbMsgAccess:
      //
      // P2SB msg only supports DWORD access. To make 8-bit wide read work we align the
      // address to neares DWORD and read that. Later on we mask the value we got from DWORD to only
  ...
  )
{
  ...
  switch (Private->AccessMethod) {
    case P2SbMmioAccess:
      Offset = Offset + Private->Offset;
      ASSERT (PCH_PCR_BASE_ADDRESS);
      MmioWrite16 (P2SB_PCR_ADDRESS (PCH_PCR_BASE_ADDRESS, Private->P2SbPid, Offset), Value);
      //
      // Readback to flush the write cycle and ensure ordering with primary
      //
      return MmioRead16 (P2SB_PCR_ADDRESS (PCH_PCR_BASE_ADDRESS, Private->P2SbPid, Offset));
...
#endif
      ASSERT (PCH_PCR_BASE_ADDRESS);
      return MmioRead32 (P2SB_PCR_ADDRESS (PCH_PCR_BASE_ADDRESS, Private->P2SbPid, Offset));
    case P2SbMsgAccess:
...
#endif
      ASSERT (PCH_PCR_BASE_ADDRESS);
      MmioWrite32 (P2SB_PCR_ADDRESS (PCH_PCR_BASE_ADDRESS, Private->P2SbPid, Offset), Value);
      //
      // Readback to flush the write cycle and ensure ordering with primary
      //
      return MmioRead32 (P2SB_PCR_ADDRESS (0xE0000000, Private->P2SbPid, Offset));
    case P2SbMsgAccess:
      P2SBBaseAddress = PCI_SEGMENT_LIB_ADDRESS (0, 0, PCI_DEVICE_NUMBER_PCH_P2SB, PCI_FUNCTION_NUMBER_PCH_P2SB, 0);
      Status = P2SbSbiExecutionEx (
                 P2SBBaseAddress,
                 Private->P2SbPid,
                 Offset,
  ...
  )
{
  ...
  P2SbController->PciCfgBaseAddr = PCI_SEGMENT_LIB_ADDRESS (0, 0, Dev, Fun, 0);
