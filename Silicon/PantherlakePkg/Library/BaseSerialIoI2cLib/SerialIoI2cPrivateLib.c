  ...
  )
{
  ...
  if (*Frequency == 400) {
    //
    // Fast Speed: frequency at 400kHz
    //
...
/**
  Write the given TargetAddress to the I2C Target Address Register
  ...
  )
{
  ...
  MemTar.Data32 = 0x0;
  MemTar.Fields.IcTar = TargetAddress & 0x3FF; // Mask 10 bits

  //
  // Check if current mode is 10 bit or 7 bit
  //
  MemTar.Fields.Ic10bHost = 0;
  if (TargetAddress & 0x80000000) {
    MemTar.Fields.Ic10bHost = 1;
  }
  ...
  )
{
  ...
  SerialIoI2cClearErrors (MmioBase);
  Status = SerialIoI2cSetFrequency (MmioBase, Frequency, TimingConfig);
  if (Status != EFI_SUCCESS) {
    return Status;
  }
  SerialIoI2cSetTargetAddress (MmioBase, TargetAddress);
  ...
  )
{
  ...
  DEBUG ((DEBUG_ERROR, "TxAbortSource.Fields.AbrtHostDis %d\n", TxAbortSource.Fields.AbrtHostDis));
