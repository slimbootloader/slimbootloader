...
typedef union {
  struct {
    UINT32  HostMode     :1;    //[RW][0] This bit controls whether I2C host is enabled.
                                  //    0: Reserved
                                  //    1: Host Enabled
    UINT32  Speed          :2;    //[RW][1:2] These bits control at which speed the I2C operates; its setting is relevant only if
                                  //  one is operating the I2C in host mode
                                  //    01: Standard mode (0 to 100 kbit/s)
                                  //    10: Fast mode (<= 400 kbit/s)
                                  //    11: High Speed Mode (<= 3.4 Mbit/s)
    UINT32  Ic10bTarget     :1;    //[RW][3] When acting as a target, this bit controls whether the DW_apb_i2c responds to 7- or 10-bit addresses.
    UINT32  Ic10bHost    :1;    //[RO][4] Identifies if I2C operates in 7 or 10 bit addressing
                                  //    0: 7-bit addressing
                                  //    1: 10-bit addressing
    UINT32  IcRestartEn    :1;    //[RW][5] Determines whether RESTART conditions may be sent when I2C is acting as a host.
                                  //    0: Restart disable
                                  //    1: Restart enable
    UINT32  IcTargetDevDis     :1;    //[RW][6] This bit controls whether I2C has its Target Device disabled.
                                  //    0:Reserved
                                  //    1: target is disabled
    UINT32  StopDetTarget   :1;    //[RW][7] Dependencies: This register bit value is applicable in the target mode only (MASTER_MODE = 1b0)
    ...
    UINT32  StopDetMstConf :1;    //[RO][10] In Host mode
                                  //    1: Issues the STOP_DET interrupt only when the host is active
                                  //    0: Issues the STOP_DET irrespective of whether the host is active
...
typedef union {
  struct {
    UINT32  IcTar          :10;   //[RW][0:9] The target address for any host transaction.
    ...
    UINT32  Ic10bHost    :1;    //[RW][12] This bit controls whether the I2C starts its transfers in 7-or 10-bit addressing mode when acting as a host.
                                  //    0: 7 bit addressing
                                  //    1: 10-bit addressing
...
typedef union {
  struct {
    ...
    UINT32  Cmd           :1;    //[WO][8] This bit controls whether a read or a write is performed when I2C acts as a host
                                  //    1: Read.
                                  //    0: Write
                                  //  When a command is entered in the TX FIFO, this bit distinguishes the write and read commands.
                                  //  In target-receiver mode:  this bit is irrelevant
                                  //  In target-transmitter mode:
...
typedef union {
  struct {
    UINT32  IcFsSclHcnt    :16;    //[RW][0:15] This register sets the SCL clock high-period count for fast speed.
                                   //           It is used in high-speed mode to send the Host Code and START BYTE or General CALL.
...
typedef union {
  struct {
    ...
    UINT32  RTxEmpty       :1;    //[RO][4] Bheavior differs based on the TX_EMPTY_CTRL selection in the IC_CON register.
                                  ...
                                  //      1: There is activity in host or target state machines
    UINT32  RRdReq         :1;    //[RO][5] DW_apb_i2c target with another I2C host status
                                  //      1: Set to 1 when DW_apb_i2c is acting as a target and another I2C host is attempting
    ...
    UINT32  RRxDone        :1;    //[RO][7] When the DW_apb_i2c is acting as a target-transmitter, set if
                                  //        the host does not acknowledge a transmitted byte, which occurs if transmission is done
    ...
    UINT32  RRestartDet    :1;    //[RO][12] Indicates whether a RESTART condition has occurred on the I2C interface if
                                  //  1. DW_apb_i2c is operating in target mode
                                  //  2. The target is the addressed target
                                  //  3. IC_SLV_RESTART_DET_EN = 1
    UINT32  RMstrOnHold    :1;    //[RO][13] Indicates whether a host is holding the bus and the TX FIFO is empty.
...
typedef union {
  struct {
    ...
    UINT32  BusHold        :1;    //[RW][2] In Host mode
...
typedef union {
  struct {
    ...
    UINT32  MstActivity    :1;    //[RO][5] Host FSM Activity Status
                                  //    1: Host FSM is not in IDLE state so the Host part of DW_apb_i2c is Active
                                  //    0: Host FSM is in IDLE state so the Host part of DW_apb_i2c is not Active
    UINT32  SlvActivity    :1;    //[RO][6] Target FSM Activity Status.
                                  //    1: Target FSM is not in IDLE state so the Target part of DW_apb_i2c is Active.
                                  //    0: Target FSM is in IDLE state so the Target part of DW_apb_i2c is not Active
...
// - The target bulk transmit mode is aborted
...
typedef union {
  struct {
    UINT32  Abrt7bAddrNoAck     :1;    //[RO][0] 1: Host in 7-bit address mode and
                                       //           the address sent was not acknowledged by any target.
    UINT32  Abrt10bAddr1NoAck   :1;    //[RO][1] 1: Host in 10-bit address mode and
                                       //           the first 10-bit address byte was not acknowledged by any target.
    UINT32  Abrt10bAddr2NoAck   :1;    //[RO][2] 1: Host is in 10-bit address mode and
                                       //           the second address byte of the 10-bit address was not acknowledged by any target.
    UINT32  AbrtTxDataNoAck     :1;    //[RO][3] 1: Host mode only: Host has received an acknowledgement for the
                                       //           address, but when it sent data byte(s) following the address, it did not receive an
                                       //           acknowledge from the remote target(s).
    UINT32  AbrtGCallNoAck      :1;    //[RO][4] 1: DW_apb_i2c in Host mode sent a General Call and no target on bus acknowledged the General Call.
    UINT32  AbrtGCallRead       :1;    //[RO][5] 1: DW_apb_i2c in host mode sent a General Call but the user programmed the byte
                                       //           following the General Call to be a read from the bus (IC_DATA_CMD[9] is set to 1).
    UINT32  AbrtHsAckDet        :1;    //[RO][6] 1: Host is in High Speed mode and the High Speed Host code was acknowledged (wrong behavior)
    UINT32  AbrtSbAckDet        :1;    //[RO][7] 1: Host has sent a START Byte and the START Byte was acknowledged (wrong behavior).
    UINT32  AbrtHsNoRst         :1;    //[RO][8] 1: The restart is disabled (IC_RESTART_EN bit (IC_CON[5]) = 0) and the user is
                                       //           trying to use the host to transfer data in High Speed mode.
    ...
    UINT32  Abrt10bRdNoRst      :1;    //[RO][10] 1: The restart is disabled (IC_RESTART_EN bit (IC_CON[5]) = 0) and the host
                                       //            sends a read command in 10-bit addressing mode.
    UINT32  AbrtHostDis       :1;    //[RO][11] 1: User tries to initiate a Host operation with the Host mode disabled.
    UINT32  ArbLost             :1;    //[RO][12] 1: Host has lost arbitration
                                       //            if IC_TX_ABRT_SOURCE[14] is also set, then the target transmitter has lost arbitration.
    UINT32  AbrtSlFlushTxFifo   :1;    //[RO][13] 1: Target has received a read command and some data exists in the TX FIFO so the
                                       //            target issues a TX_ABRT interrupt to flush old data in TX FIFO.
    UINT32  AbrtSlvArbLost      :1;    //[RO][14] 1: Target lost the bus while transmitting data to a remote host.
    UINT32  AbrtSlvRdIntx       :1;    //[RO][15] 1: Occurs when the processor side responds to a target mode request for data to be transmitted
                                       //            to a remote host and user writes a 1 in CMD (bit 8) of IC_DATA_CMD register.
    UINT32  AbrtUserAbrt        :1;    //[RO][16] Host only: Host has detected the user initiated transfer abort (IC_ENABLE[1])
...
typedef union {
  struct {
    ...
    UINT32  SlvDisWBusy    :1;    //[RO][1] This bit indicates if a potential or active Target operation has been aborted due to the
                                  //        setting of the IC_ENABLE register from 1 to 0. This bit is set when the CPU writes a 0
    UINT32  SlvRxDataLost  :1;    //[RO][2] This bit indicates if a Target-Receiver operation has been aborted with at least one data
...
/**
  ...
  @retval   EFI_SUCCESS           I2C successfully set frequency
  @retval   EFI_DEVICE_ERROR      I2C did not support the given frequency
**/
EFI_STATUS
EFIAPI
...
/**
  Write the given target address to the I2C Target Address Register

  @param[in]   MmioBase       MMIO Base Address of specified I2C controller
  @param[in]   TargetAddress   Address of the end point device
**/
VOID
SerialIoI2cSetTargetAddress (
  IN UINTN   MmioBase,
...
/**
  ...
  Before any transaction:
   ...
   5. Write target address to I2C target address register
   6. Initiate I2C Controller

  @param[in]   MmioBase        MMIO Base Address of specified I2C controller
  @param[in]   Target Address  -Address of the end point device
  @param[in]   *Frequency      Pointer to bus clock frequency (in kHz)
