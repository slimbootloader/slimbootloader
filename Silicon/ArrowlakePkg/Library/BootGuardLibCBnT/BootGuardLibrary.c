  ...
  )
{
  ...
  if (IsBootGuardSupported ()) {
    ...
    ///
    MeFwSts = MmioRead32 (HeciBaseAddress + R_ME_HFS);
    DEBUG ((DEBUG_INFO, "ME FW STS = %x\n", MeFwSts));
    MeFwSts2 = MmioRead32 (HeciBaseAddress + R_ME_HFS_2);
    DEBUG ((DEBUG_INFO, "ME FW STS 2 = %x\n", MeFwSts2));
    MeFwSts3 = MmioRead32 (HeciBaseAddress + R_ME_HFS_3);
