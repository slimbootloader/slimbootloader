  ...
  )
{
  ...
  if (Controller->SbRegBar != 0) {
    ...
    if (PciRead32 (Controller->P2sbBase) == 0xFFFFFFFF) {
      DEBUG ((DEBUG_ERROR, "P2SB (0x%x) is hidden and SBREG_BAR is unknown.\n", Controller->P2sbBase));
      ASSERT (FALSE);
    }
    P2sbBase = (PciRead32 (Controller->P2sbBase + 0x10) & 0xFFFFFFF0) | PciRead32 (Controller->P2sbBase + 0x14);
    if (P2sbBase == 0) {
  ...
  )
{
  ...
  Data32 = P2sbRead32 (P2sbBase, (P2SB_PID)Community->Pid, 0, RegOffset);
  return Data32;
}
