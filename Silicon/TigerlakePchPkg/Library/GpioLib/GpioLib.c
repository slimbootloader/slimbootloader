  ...
  )
{
  ...
  Data32Or  = (UINT32) ((GpeDw2Value << N_PCH_PWRM_GPIO_CFG_GPE0_DW2) |
                        (GpeDw1Value << N_PCH_PWRM_GPIO_CFG_GPE0_DW1) |
                        (GpeDw0Value << N_PCH_PWRM_GPIO_CFG_GPE0_DW0));

  MmioAndThenOr32 (
    (PCH_PWRM_BASE_ADDRESS + R_CNL_PCH_PWRM_GPIO_CFG),
    Data32And,
    Data32Or
    );
}
  ...
  )
{
  ...
  DEBUG((DEBUG_INFO, "GPIO Address: 0x%08x, PadCfgAndMask = 0x%08x, PadCfgOrMask = 0x%08x\n", PCH_PCR_ADDRESS(GpioGroupInfo[GroupIndex].Community, PadCfgReg), PadCfgAndMask, PadCfgOrMask));
  if (PadCfgLock) {
  ...
  )
{
  ...
  if (!SupportedValue) {
    DEBUG ((DEBUG_WARN, "GPIO WARNING: GPIO pad (Group=%d, Pad=%d) %dus debounce time rounded down to %dus\n",
            GpioGetGroupIndexFromGpioPad (GpioPad),
            GpioGetPadNumberFromGpioPad (GpioPad),
            *DebounceTime,
