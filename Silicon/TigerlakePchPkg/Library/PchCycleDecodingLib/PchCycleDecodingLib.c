  ...
  )
{
  ...
  if ((Address & (~B_LPC_CFG_LGMR_MA)) != 0) {
  ...
  )
{
  ...
  //
  EspiPcbc = PciRead32 (LpcBase + R_ESPI_CFG_PCBC);
  if (IsPchH () && ((EspiPcbc & B_ESPI_CFG_PCBC_ESPI_EN) != 0)) {
    return EFI_UNSUPPORTED;
  }
  ...
  )
{
  ...
  LpcBase = LpcPciBase ();
  *Address = PciRead32 (LpcBase + R_LPC_CFG_LGMR) & B_LPC_CFG_LGMR_MA;
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  if (PchDmiIsBootBiosStrapSetForSpi ()) {
    ...
    //
    PciWrite16 (BaseAddr + R_SPI_CFG_BDE, BiosDecodeEnable);
  } else {
    BaseAddr = LpcPciBase ();
    //
    // If LPC/eSPi, program LPC offset D8h to BiosDecodeEnable.
    //
    PciWrite16 (BaseAddr + R_LPC_CFG_BDE, BiosDecodeEnable);
  }
