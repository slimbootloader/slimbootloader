  ...
  )
{
  ...
  return PciRead16 (LpcBaseAddress + PCI_DEVICE_ID_OFFSET);
}
  ...
  )
{
  ...
  RevId = PciRead8 (LpcBaseAddress + PCI_REVISION_ID_OFFSET);
  ...
  )
{
  ...
  LpcDeviceId = PciRead16 (LpcBaseAddress + PCI_DEVICE_ID_OFFSET);
  LpcVendorId = PciRead16 (LpcBaseAddress + PCI_VENDOR_ID_OFFSET);
...
/**
  Check if this is CML PCH generation

  @retval TRUE                It's CML PCH
  @retval FALSE               It's not CML PCH
**/
BOOLEAN
IsCmlPch (
  VOID
