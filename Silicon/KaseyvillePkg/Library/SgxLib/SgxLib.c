...
)
{
  ...
  for (Index = 0; Index < SgxManifest->ArraySize; Index++) {
    ...
    if (Status == EFI_NOT_FOUND) {
      ...
      FreePages (CctVsTempNode, EFI_SIZE_TO_PAGES (sizeof (CCT_VS_METADATA_NODE)));
      Status = EFI_SUCCESS;
      continue;
    }
...
)
{
  ...
  if (PlatformData != NULL) {
    ...
    if (Status == EFI_BUFFER_TOO_SMALL && DataSize > 0) {
      ...
      if (SgxTempState == NULL) {
        DEBUG ((DEBUG_ERROR, "%a() Failed to allocate memory for variable data! Exiting.\n", __func__));
        goto Exit;
      }
      Status = UefiGetVariable (gSgxUefiDisabledScenario,
                              &gSgxUefiDisabledScenarioGuid,
