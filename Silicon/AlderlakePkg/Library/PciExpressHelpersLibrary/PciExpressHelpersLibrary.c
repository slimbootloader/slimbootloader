...
{
  return (UINT8) ((PciSegmentRead16 (RpBase + R_PCIE_CFG_LSTS) & B_PCIE_LSTS_NLW) >> N_PCIE_LSTS_NLW);
}
...
{
  return PciSegmentRead16 (RpBase + R_PCIE_CFG_LSTS) & B_PCIE_LSTS_CLS;
}
  ...
  )
{
  ...
  if (IsAdlPch () && IsPchS () && (PchStepping () == PCH_A0)) {
