...
/**
  ...
  exact match
**/
#define FUSA_INFO_VERSION 0x00000104
...
#define FUSA_TOTAL_NUMBER_OF_TESTS    83
  ...
  )
{
  ...
  CheckCount = MIN (pFusaTestResult->TotalChecks, ARRAY_SIZE (pFusaTestResult->CheckResults));
  ...
  )
{
    ...
    if (!EFI_ERROR (Status)) {
      ...
      if (!EFI_ERROR (Status)) {
        ...
        if (Status == EFI_SUCCESS) {
          DEBUG ((DEBUG_VERBOSE, "Reading FuSa Startup BIST pattern from address %p, Length 0x%X\n", FusaStartupBistPattern, FusaCompLength));
  ...
  if (PowerCfgData != NULL) {
    switch (PowerCfgData->BiProcHot) {
      case 1:
        FspsConfig->BiProcHot = 1;
        FspsConfig->DisableProcHotOut= 0;
        break;
      case 2:
        FspsConfig->BiProcHot = 1;
        FspsConfig->DisableProcHotOut= 1;
        break;
      case 0:
      default:
