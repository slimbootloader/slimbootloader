  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "CPU CrashLog data are not collected, Status = %r\n", Status));
  }
  ...
  )
{
  ...
  if (MainLogSize) {
    DEBUG ((DEBUG_INFO, "Cpu CrashLog data is present\n"));
    ...
    DEBUG ((DEBUG_INFO, "Cpu CrashLog data is not present\n"));
