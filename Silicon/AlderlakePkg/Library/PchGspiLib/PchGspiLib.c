...
)
{
  ...
  if (Address == 0) {
    ...
    BarType = PciSegmentRead32 (mPchGspiCtrls[Controller] + PCI_BASE_ADDRESSREG_OFFSET);
    PciSegmentWrite32 (mPchGspiCtrls[Controller] + PCI_BASE_ADDRESSREG_OFFSET, (UINT32)Address);
    // For a 64-bit BAR, clear the upper dword to prevent any stale value from
    // combining with the programmed base address and producing a wrong MMIO target.
    if (((BarType & BIT0) == 0) && ((BarType & (BIT2 | BIT1)) == BIT2)) {
      PciSegmentWrite32 (mPchGspiCtrls[Controller] + PCI_BASE_ADDRESSREG_OFFSET + 4, 0);
    }
  }
