...
#include <Library/HeciInitLib/MeChipset.h>
#include <Library/HeciInitLib/HeciCore.h>
  ...
  )
{
  ...
  //
  Status = TpmHashAndExtendPcrEventLog (0, (UINT8 *)(UINTN)HerValue, HashDataLen, EV_NONHOST_INFO, EventLogSize, (UINT8*)(UINTN)&CsmeEvent);
  ...
  )
{
  ...
  HeciMemBar =  HeciPciRead32 (HECI_FUNCTION_NUMBER, PCI_BASE_ADDRESSREG_OFFSET) & 0xFFFFFFF0;
  if ((HeciPciRead32 (HECI_FUNCTION_NUMBER, PCI_BASE_ADDRESSREG_OFFSET) & B_PCI_BAR_MEMORY_TYPE_MASK) == B_PCI_BAR_MEMORY_TYPE_64) {
    HeciMemBar += LShiftU64((UINT64) HeciPciRead32 (HECI_FUNCTION_NUMBER, (PCI_BASE_ADDRESSREG_OFFSET + 4)), 32);
  }
  ...
  ///
  if ((MmioRead32 ((UINTN) HeciMemBar + D0I3C) & BIT2) == BIT2) {
  ...
  /// Read HER Status register value
  ///
  Enh_Hers = MmioRead32 ((UINTN) HeciMemBar + R_HERS);
  //DEBUG((DEBUG_INFO, "Enh_Hers %x Hers %x \n", Enh_Hers, Hers));
  ...
  if ((Enh_Hers & B_ME_EXTEND_REG_ALGORITHM) == V_ME_SHA_384) {
    HashAlgorithm = V_ME_SHA_384;
    HerMaxRegCount = (UINT8) HER_REG_COUNT_SHA384;
  } else if ((Enh_Hers & B_ME_EXTEND_REG_ALGORITHM) == V_ME_SHA_256) {
