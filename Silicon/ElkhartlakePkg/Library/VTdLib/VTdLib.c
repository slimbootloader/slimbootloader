...
**/
UINT16  mDevEnMap[][2] = {{0x0200, 0x80}, {0x0500, 0x80}, {0x1400, 0x80}, {0x1401, 0x80}, {0x0700, 0x80}, {0x0701, 0x80}, {0x0702, 0x80}, {0x0703, 0x80}, {0x0704, 0x80}, {0x0705, 0x80}};
...
#define V_SA_PCI_DEV_2_GT0_JSL_EHL_ID        0x4569 ///< Dev2 JSL/EHL GT0
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "DMAR flag: 0x%x final update\n", DmarTable->DmarHeader.Flags));
  ///
  /// Get OemId
  ///
  CopyMem (DmarTable->DmarHeader.Header.OemId, "INTEL ", sizeof (DmarTable->DmarHeader.Header.OemId));
  DmarTable->DmarHeader.Header.OemTableId      = 0x746F6F626D696C53;
  ...
  )
{
  ...
  ///
  if ((PciRead32 (PCI_LIB_ADDRESS (SA_MC_BUS, 0, 0, R_SA_MC_CAPID0_A_OFFSET)) & BIT23)) {
  ...
  if (SiCfgData != NULL) {
    ...
    DEBUG ((DEBUG_INFO, "mInterruptRemappingSupport: %d\n",mInterruptRemappingSupport));
  }
