  ...
  )
{
  ...
  *(DriverParams.OutputFunc) = NULL;
  ...
  if (FixedPcdGetBool(PcdIoeCsmeUpdateEnabled)) {
    if (CsmeUpdateApi->FwuSetTarget == NULL){
      DEBUG((DEBUG_ERROR, "ERROR:FwuSetTarget is null\n"));
      return EFI_UNSUPPORTED;
    }
    Signature    = (UINT32)ImageHdr->UpdateHardwareInstance;
    UpdateStatus = EFI_INVALID_PARAMETER;
    if (Signature == FW_UPDATE_COMP_CSME_REGION) {
      ...
      DEBUG((DEBUG_ERROR, "Unsupported signature:0x%x\n", Signature));
    }
    if (UpdateStatus != SUCCESS) {
      return EFI_DEVICE_ERROR;
    }
