...
def gen_fsp_upd_yaml_files(fsp_path, fv_dir, cfg_dir,
    ...
    # Emit an FSP_X_CONFIG-in-UPD offset marker as a YAML comment so
    # stitch-time delta regeneration (GenFspUpdDelta.py) can rebuild the
    # UPD-relative delta offsets without needing FSP binaries or headers.
    # Runtime fields (FspUpdHeader, FspXArchUpd, terminators) are excluded
    # from the CFGDATA payload, so the first active field's offset is the
    # offset of FSP_X_CONFIG within the full FSP{M,S}_UPD buffer.
    fspm_active = [f for f in fspm_fields if f.name not in FSPM_RUNTIME_FIELDS]
    fsps_active = [f for f in fsps_fields if f.name not in FSPS_RUNTIME_FIELDS]
    fspm_cfg_base = min(f.offset for f in fspm_active) if fspm_active else 0
    fsps_cfg_base = min(f.offset for f in fsps_active) if fsps_active else 0

    fspm_marker = '#  @fspm_cfg_base: 0x%X\n' % fspm_cfg_base
    fsps_marker = '#  @fsps_cfg_base: 0x%X\n' % fsps_cfg_base

    def _inject_cfg_base_marker(text, marker):
        # No-op if the marker is already present (idempotent regen).
        if marker in text:
            return text
        # Preferred anchor: insert right after the generator banner so
        # the marker sits near the top with the other file-scope headers.
        anchor = '#  Generated by GenFspUpdYaml.py - DO NOT EDIT MANUALLY\n'
        if anchor in text:
            return text.replace(anchor, anchor + marker, 1)
        # Anchor missing (e.g. banner reformat) -- prepend the marker so
        # downstream YAML-only delta regeneration can still locate it.
        return marker + text

    fspm_yaml_text = _inject_cfg_base_marker(fspm_yaml_text, fspm_marker)
    fsps_yaml_text = _inject_cfg_base_marker(fsps_yaml_text, fsps_marker)
    ...
    print('  Generated %s (cfg_base=0x%X)' % (fspm_yaml, fspm_cfg_base))
    print('  Generated %s (cfg_base=0x%X)' % (fsps_yaml, fsps_cfg_base))
