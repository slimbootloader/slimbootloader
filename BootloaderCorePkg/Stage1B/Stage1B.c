...
/**
  Check digest for external hash store component.

  @param[in] Stage1bParam   Stage1B parameters.
  @param[in] Src            Extrenal hash store address
  @param[in] Length         Used length of external hash store

**/
VOID
CheckExtHashStoreMbDigestExtend (
    IN STAGE1B_PARAM   *Stage1bParam,
    IN UINT8           *Src,
    IN UINT32           Length
  )
{
  // Calculate External hash srore Digest if PcdMeasuredBootHashMask differs with PcdCompSignHashAlg
  if (GetCryptoHashAlg(PcdGet32(PcdMeasuredBootHashMask)) != PcdGet8(PcdCompSignHashAlg)){
    CalculateHash ((UINT8 *)Src, Length, GetCryptoHashAlg(PcdGet32(PcdMeasuredBootHashMask)),
                                                        (UINT8 *) Stage1bParam->KeyHashManifestHash);

  } else {
    // Use Stage1bParam->KeyHashManifestHash generated by RsaVerify()
  }
  ...
  )
{
  ...
  if (MEASURED_BOOT_ENABLED()) {
    ...
    CheckExtHashStoreMbDigestExtend(Stage1bParam, (UINT8 *)OemKeyHashBlob, OemKeyHashBlob->UsedLength);
  }
  ...
  )
{
  ...
  if (MEASURED_BOOT_ENABLED() ) {
    if (GetBootMode() != BOOT_ON_S3_RESUME) {
        ...
        //Extend Key manifest hash
        if (Stage1bParam->KeyHashManifestHashValid == 1) {

          if (PcdGet32(PcdMeasuredBootHashMask) == HASH_ALG_SHA256) {
            TpmAlg = TPM_ALG_SHA256;
          } else if (PcdGet32(PcdMeasuredBootHashMask) == HASH_ALG_SHA384) {
            TpmAlg = TPM_ALG_SHA384;
          } else if (PcdGet32(PcdMeasuredBootHashMask) == HASH_ALG_SM3_256) {
            TpmAlg = TPM_ALG_SM3_256;
          }
          TpmExtendPcrAndLogEvent (0, TpmAlg, Stage1bParam->KeyHashManifestHash,
                      EV_POST_CODE, sizeof("Key Manifest"), (UINT8 *)"Key Manifest");
        }
    }
  }
